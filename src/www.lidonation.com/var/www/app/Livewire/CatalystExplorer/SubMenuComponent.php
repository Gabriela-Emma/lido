...
class SubMenuComponent extends Component
{
    ...
    {
        $this->model = Group::where('slug', $this->path)->first();
    ...
    protected function buildBreadCrumbs(): static
    {
        ...
        if ($this->section && $this->section !== 'dashboard' && Route::has("catalyst-explorer.{$routeName}")) {
            $this->crumbs->push([
                'label' => Str::title(Str::replace('-', ' ', $this->section)),
                'link' => route("catalyst-explorer.{$routeName}"),
