...
if (!proposal.value?.vote) {
    axios.get(route('catalyst-explorer.votes.index', { proposal: props.proposal.id }), {})
        .then((res) => {
...
function vote(vote: VOTEACTIONS, proposal: Proposal) {
    if (proposal.vote) {
        router.patch(
            route('catalyst-explorer.votes.update', { vote: proposal.vote }),
            { vote },
            {
                preserveScroll: true,
                preserveState: true,
        ...
        router.post(
            route('catalyst-explorer.votes.store'),
