        ...
        <div class="-mt-10 flex gap-8">
            <div class="flex" v-if="prev">
                <a href="#" v-if="prev?.url" @click.prevent="paginate(prev.page)"
                   class="inline-flex items-center border-t-2 border-transparent pt-8 pr-1 text-sm font-medium text-slate-500 hover:border-yellow-500 hover:text-yellow-500">
                ...
                <span v-else
                      class="inline-flex items-center border-t-2 border-transparent pt-8 pr-1 text-sm font-medium text-slate-400">
                    <svg class="mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                         fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd"
                              d="M18 10a.75.75 0 01-.75.75H4.66l2.1 1.95a.75.75 0 11-1.02 1.1l-3.5-3.25a.75.75 0 010-1.1l3.5-3.25a.75.75 0 111.02 1.1l-2.1 1.95h12.59A.75.75 0 0118 10z"
            ...
            <div class="hidden md:-mt-px md:flex">
                ...
                <span
                    class="inline-flex items-center border-t-2 border-transparent px-4 pt-8 text-sm font-medium border-teal-500 text-teal-600"
                    v-if="!!link.active">{{ link.label }}</span>
                    <span href="#" v-else-if="link?.label === '...'"
                          class="inline-flex items-center border-t-2 border-transparent px-4 pt-8 text-sm font-medium text-slate-500">
                    {{ link.label }}
                </span>
                    <a href="#" v-else @click.prevent="paginate(link.page)"
                       class="inline-flex items-center border-t-2 border-transparent px-4 pt-8 text-sm font-medium text-slate-500 hover:border-yellow-500 hover:text-yellow-500">
                        {{ link.label }}
                    </a>
                </template>
            </div>
            ...
            <div class="flex" v-if="next">
                <a href="#" v-if="next?.url" @click.prevent="paginate(next.page)"
                   class="inline-flex items-center border-t-2 border-transparent pt-8 pl-1 text-sm font-medium text-slate-500 hover:border-yellow-500 hover:text-yellow-500">
                    Next
                ...
                <span v-else
                      class="inline-flex items-center border-t-2 border-transparent pt-8 pl-1 text-sm font-medium text-slate-400">
                Next
                    <!-- Heroicon name: mini/arrow-long-right -->
                <svg class="ml-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                     fill="currentColor" aria-hidden="true">
