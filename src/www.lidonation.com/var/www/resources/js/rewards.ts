...
import Layout from "./reward/Shared/Layout.vue";
import {createPinia} from "pinia";
import {marked} from 'marked';
import HeaderComponent from './reward/Shared/Components/HeaderComponent.vue';
...
createInertiaApp({
    ...
    resolve: name => {
        const page = require(`./reward/Pages/${name}`).default;
        page.layout ??= Layout;
    ...
    setup({el, App, props, plugin}) {
        ...
        const app = createApp({ render: () => h(App, props) })
            .use(plugin)
            .use(modal, {
                resolve: (name) => import(`./reward/Pages/${name}`),
            })
            .use(PrimeVue)
            .use(timeago)
            .use(pinia)
