<div class="relative" wire:key="{{$catalystUser->id}}">
    <div wire:loading.class.remove="hidden" wire:loading.delay.shortest.class="absolute" wire:target="toggleOwnMetrics" class="sticky left-0 z-10 flex items-center justify-center hidden w-full h-0 p-0 overflow-visible top-1/2">
        <div class="flex items-center justify-center w-24 h-24 p-3 bg-white rounded-full xl:w-40 xl:h-40 lg:h-32 lg:w-32 bg-opacity-80">
            <svg class="relative w-8 h-8 border-t-2 border-b-2 rounded-full lg:w-16 lg:h-16 animate-spin border-primary-600" viewBox="0 0 24 24"></svg>
        </div>
    </div>
    ...
    <div class="flex flex-row justify-end font-semibold text-yellow-400 right-0top-0p-4">
        <div class="flex items-center" x-data="{ownMetrics: @entangle('ownMetrics')}">
            <button type="button" :class="{'bg-teal-800': ownMetrics, 'bg-gray-200': !ownMetrics}" wire:click="toggleOwnMetrics({{ $catalystUser->id }})" class="relative inline-flex flex-shrink-0 h-6 transition-colors duration-200 ease-in-out bg-gray-200 border-2 border-transparent rounded-full cursor-pointer w-11 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500" role="switch" aria-checked="false" aria-labelledby="annual-billing-label">
                <span aria-hidden="true" :class="{'translate-x-5': ownMetrics, 'translate-x-0': !ownMetrics}" class="inline-block w-5 h-5 transition duration-200 ease-in-out transform translate-x-0 bg-white rounded-full shadow pointer-events-none ring-0"></span>
            </button>
                ...
                <div class="flex flex-row justify-start p-1 overflow-x-scroll flex-nowrap categories">
                    <div class="border border-gray-300 -mt-px -ml-px inline-flex flex-col gap-6 justify-between border-opacity-50 p-4 min-w-15 md:min-w-[13rem] xl:min-w-[initial]">
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            ...
                            <div class="relative w-full xl:w-48" wire:ignore>
                                <x-catalyst.users.chart-per-fund chartName="allTimeCompletedPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeCompletedPerRound')?->labels" :data="$this->getMetric('allTimeCompletedPerRound')?->data"></x-catalyst.users.chart-per-fund>
                            </div>
                            @else
                    ...
                    <div class="border border-gray-300 -mt-px -ml-px inline-flex flex-col gap-6 justify-between border-opacity-50 p-4 min-w-15 md:min-w-[13rem] xl:min-w-[initial]">
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            ...
                            <div class="relative w-full xl:w-48" wire:ignore>
                                <x-catalyst.users.chart-per-fund chartName="allTimeFundedPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeFundedPerRound')?->labels" :data="$this->getMetric('allTimeFundedPerRound')?->data"></x-catalyst.users.chart-per-fund>
                            </div>
                            @else
                            <div class="inline-flex min-w-[11rem]">
                                <div class="relative z-0 inline-flex rounded-md shadow-sm">
                    ...
                    <div class="border border-gray-300 -mt-px -ml-px inline-flex flex-col gap-6 justify-between border-opacity-50 p-4 min-w-15 md:min-w-[13rem] xl:min-w-[initial]">
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            ...
                            <div class="relative w-full xl:w-48" wire:ignore>
                                <x-catalyst.users.chart-per-fund chartName="allTimeProposedPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeProposedPerRound')->labels" :data="$this->getMetric('allTimeProposedPerRound')->data"></x-catalyst.users.chart-per-fund>
                            </div>
                            @else
                            <div class="inline-flex min-w-[11rem]">
                                <div class="relative z-0 inline-flex rounded-md shadow-sm">
