                ...
                <div class="flex flex-row justify-start p-1 overflow-x-scroll flex-nowrap categories">
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            @if ($this->getMetric('allTimeCompletedPerRound')?->data->count() > 1)
                                <div class="relative w-full xl:w-48" wire:ignore>
                                    <x-catalyst.users.chart-per-fund chartName="allTimeCompletedPerRound"
                                        :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeCompletedPerRound')?->labels"
                    ...
                    <div
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            @if ($this->getMetric('allTimeFundedPerRound')?->data->count() > 1)
                                <div class="relative w-full xl:w-48" wire:ignore>
                                    <x-catalyst.users.chart-per-fund chartName="allTimeFundedPerRound" :modelId="$catalystUser->id"
                                        :labels="$this->getMetric('allTimeFundedPerRound')?->labels" :data="$this->getMetric('allTimeFundedPerRound')?->data"></x-catalyst.users.chart-per-fund>
                                </div>
                            @else
                                <div class="inline-flex min-w-[11rem]">
                                    <div class="relative z-0 inline-flex rounded-md shadow-sm">
                    ...
                    <div
                        ...
                        <div class="flex w-full min-w-full overflow-hidden">
                            @if ($this->getMetric('allTimeProposedPerRound')?->data->count() > 1)
                                <div class="relative w-full xl:w-48" wire:ignore>
                                    <x-catalyst.users.chart-per-fund chartName="allTimeProposedPerRound"
                                        :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeProposedPerRound')->labels"
                    ...
                    @if ($this->getMetric('allTimeFundingPerRound')?->data?->sum() > 0)
                        <div
                            class="border border-gray-300 -mt-px -ml-px inline-flex flex-col gap-6 justify-between border-opacity-50 p-4 min-w-15 md:min-w-[13rem] xl:min-w-[initial]">
                            <div
                                class="flex flex-row flex-no-wrap items-center justify-between gap-4 md:justify-start">
                              <div class="flex-col flex-wrap font-semibold leading-2" x-data>
                                    @if ($this->getMetric('allTimeFundingPerRound')?->totalAda)
                                        <div>
                                            <span class="font-semibold ">
                                                ₳{{ humanNumber($this->getMetric('allTimeFundingPerRound')?->totalAda) }}
                                            </span>
                                        </div>
                                    @endif
                                    @if ($this->getMetric('allTimeFundingPerRound')?->totalUsd)
                                        <div>
                                            <span class="font-semibold">
                                                ${{ humanNumber($this->getMetric('allTimeFundingPerRound')?->totalUsd) }}
                                            </span>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="w-full min-w-full overflow-hidden">
                                @if ($this->getMetric('allTimeFundingPerRound')?->data?->count() > 1)
                                    <div class="relative w-full xl:w-48" wire:ignore>
                                        <x-catalyst.users.chart-per-fund :dataType="$this->getMetric('allTimeFundingPerRound')?->currency"
                                            chartName="allTimeFundingPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeFundingPerRound')?->labels"
                        ...
                        <div
                            ...
                            <div
                                class="flex flex-row flex-no-wrap items-center justify-between gap-5 md:justify-start text-blue-dark-500">
                                <div class="flex-col flex-wrap font-semibold leading-2" x-data>
                                    @if ($this->getMetric('allTimeAwardedPerRound')?->totalAda)
                                        <div>
                                            <span class="font-semibold ">
                                                ₳{{ humanNumber($this->getMetric('allTimeAwardedPerRound')?->totalAda) }}
                                            </span>
                                        </div>
                                    @endif
                                    @if ($this->getMetric('allTimeAwardedPerRound')?->totalUsd)
                                        <div>
                                            <span class="font-semibold">
                                                ${{ humanNumber($this->getMetric('allTimeAwardedPerRound')?->totalUsd) }}
                                            </span>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="w-full min-w-full overflow-hidden">
                                @if ($this->getMetric('allTimeAwardedPerRound')?->data?->count() > 1)
                                    <div class="relative w-full xl:w-48" wire:ignore>
                                        <x-catalyst.users.chart-per-fund :dataType="$this->getMetric('allTimeFundingPerRound')?->currency"
                                            chartName="allTimeAwardedPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeAwardedPerRound')?->labels"
                        ...
                        <div
                            ...
                            <div
                                class="flex flex-row flex-no-wrap items-center justify-between gap-5 md:justify-start text-blue-dark-500">
                                <div class="flex-col flex-wrap font-semibold leading-2" x-data>
                                    @if ($this->getMetric('allTimeReceivedPerRound')?->totalAda)
                                        <div>
                                            <span class="font-semibold ">
                                                ₳{{ humanNumber($this->getMetric('allTimeReceivedPerRound')?->totalAda) }}
                                            </span>
                                        </div>
                                    @endif
                                    @if ($this->getMetric('allTimeReceivedPerRound')?->totalUsd)
                                        <div>
                                            <span class="font-semibold">
                                                ${{ humanNumber($this->getMetric('allTimeReceivedPerRound')?->totalUsd) }}
                                            </span>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="w-full min-w-full overflow-hidden">
                                @if ($this->getMetric('allTimeReceivedPerRound')?->data?->count() > 1)
                                    <div class="relative w-full xl:w-48" wire:ignore>
                                        <x-catalyst.users.chart-per-fund :dataType="$this->getMetric('allTimeFundingPerRound')?->currency"
                                            chartName="allTimeReceivedPerRound" :modelId="$catalystUser->id" :labels="$this->getMetric('allTimeReceivedPerRound')?->labels"
