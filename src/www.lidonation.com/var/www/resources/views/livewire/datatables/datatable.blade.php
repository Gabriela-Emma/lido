    ...
    <div class="relative">
        <div class="flex justify-between items-center mb-1">
            <div class="h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-sm shadow-xs">
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                <div x-data="{ init() {
                    ...
                    <button wire:click="export" class="flex items-center space-x-2 px-3 border border-green-400 rounded-sm bg-white text-green-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-200 focus:outline-none"><span>{{ __('Export') }}</span>
        ...
        <div wire:loading.class="opacity-50" class="rounded-sm table-body-wrapper @unless($complex || $this->hidePagination) rounded-b-none @endunless shadow-xs bg-white max-w-screen overflow-x-scroll border-2 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            <div>
                <div class="table align-middle min-w-full">
                    @unless($this->hideHeader)
                    <div class="table-row divide-x divide-gray-300">
                        @foreach($this->columns as $index => $column)
                            @if($hideable === 'inline')
                                @include('datatables::header-inline-hide', ['column' => $column, 'sort' => $sort])
                            @elseif($column['type'] === 'checkbox')
                                @unless($column['hidden'])
                                    <div class=" table-cell h-12 w-32 py-4 flex justify-center overflow-hidden align-top px-6 border-b border-gray-300 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider focus:outline-none">
                    ...
                    <div class="table-row divide-x divide-blue-200 bg-blue-100">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                <div class="overflow-hidden align-top bg-blue-100 px-6 py-5 border-b border-gray-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                    <div>SELECT ALL</div>
                                    <div>
